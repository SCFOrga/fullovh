...
$req =$bdd->prepare('SELECT * FROM `ChauffJour`  ORDER BY `dateJour` DESC LIMIT 10 ');
$req ->execute();
$temp = array();
...
<body>
<div class="txt"><h3>Graphique de la consommation du <u>Chauffage</u> par jour</h3></div>
...
{
    $data[substr($temp[$i]['dateJour'], 0, 10)] = intval($temp[$i]['conso']);
}
//print_r($temp);
